...
def test_preset_max_clip_limits_clips(home, audio, tmp_path):
    ...
    sess = _session()
    presets.PRESETS.append(presets.Preset("dummy-short", "ダミー(短)", "dummy", "fake", "dummy-model", max_clip=8))
    st = pipeline.Settings(output_dir=str(tmp_path), preset="dummy-short", vad="energy", max_clip=20, batch_size=2,
